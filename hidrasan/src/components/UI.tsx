...
export const FeatureItem: React.FC<{ title: string; text: string; icon?: React.ReactNode }> = ({ title, text, icon }) => (
  <div className="flex flex-col sm:flex-row gap-4 p-6 sm:p-8 rounded-[2rem] bg-white/40 backdrop-blur-md border border-white/20 hover:bg-white/60 transition-all duration-300 group">
    <div className="flex-shrink-0 w-12 h-12 rounded-2xl bg-blue-50 flex items-center justify-center text-blue-600 group-hover:scale-110 transition-transform duration-300">
      {icon || <CheckCircle2 className="w-6 h-6" />}
    </div>
    <div>
      <h3 className="font-bold text-lg mb-1 text-zinc-950">{title}</h3>
