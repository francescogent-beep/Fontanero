...
export const Home: React.FC = () => {
  ...
  return (
    <>
      <SEO 
        title={`Instalaciones y Reformas de Fontanería en ${BUSINESS_DETAILS.address.city}`}
        description={`${BUSINESS_DETAILS.name}: Especialistas en proyectos integrales de fontanería en ${BUSINESS_DETAILS.address.city}. Reformas, sustitución de tuberías y locales comerciales. ¡Calidad garantizada!`}
      />

      {/* Hero Section */}
      <section className="relative pt-12 pb-[96px] md:pt-24 overflow-hidden">
        {/* Background Image with Overlay */}
        <div 
          className="absolute inset-0 -z-20 bg-cover bg-center bg-no-repeat bg-fixed"
          style={{ 
            backgroundImage: "url('https://i.imgur.com/6JFnvuk.jpeg')",
            filter: "brightness(0.4) saturate(0.8)"
          }}
        />
        <div className="absolute inset-0 -z-10 bg-gradient-to-b from-zinc-950/50 via-transparent to-white" />
        ...
        <Container>
          <div className="flex flex-col items-center text-center max-w-5xl mx-auto relative">
            ...
            <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[300px] md:w-[600px] h-[200px] md:h-[400px] bg-blue-400/20 blur-[80px] md:blur-[120px] rounded-full -z-10" />
            
            <Badge className="mb-6 md:mb-8 bg-white/10 backdrop-blur-md border-white/20 text-white uppercase tracking-wider text-[8px]">fontaneria en {BUSINESS_DETAILS.address.city}</Badge>
            
            <H1 className="mb-8 md:mb-10">
              <span className="text-white text-[42px] leading-tight">
                Instalaciones y Reformas con <span className="text-blue-400 drop-shadow-sm">Ejecución Profesional</span>
              </span>
            </H1>
            
            <div className="mb-10 md:mb-12 max-w-xl mx-auto bg-black/40 backdrop-blur-xl border border-white/10 px-6 py-4 md:px-8 md:py-5 rounded-2xl md:rounded-[1.5rem] shadow-xl">
              <Text size="md" className="!text-white font-bold leading-relaxed md:text-lg drop-shadow-sm">
                Especializados en reformas integrales, sustitución de redes de agua y fontanería para locales comerciales en toda la zona de {BUSINESS_DETAILS.address.city}.
              </Text>
            </div>

            <div className="flex flex-col sm:flex-row gap-4 md:gap-6 justify-center w-full sm:w-auto px-4 sm:px-0 pt-[19px]">
              <Button to="/contacto" variant="primary" className="w-full sm:w-auto px-10">Solicita Presupuesto</Button>
              <Button to="/servicios" variant="outline" className="w-full sm:w-auto px-10 bg-white/10 backdrop-blur-md border-white/20 text-white hover:bg-white/20">Nuestros Proyectos</Button>
            </div>
          </div>
        </Container>
      </section>
        ...
        <Container>
          <Grid cols={3} className="gap-4 sm:gap-6 lg:gap-10 pt-[35px] pb-0">
            <FeatureItem 
              ...
              text="Diseñamos cada instalación para evitar problemas futuros. Soluciones a largo plazo, no parches." 
              icon={<Clock className="w-6 h-6" />}
            />
            <FeatureItem 
              title="Presupuesto Detallado" 
              text="Valoración completa de tu proyecto sin sorpresas finales. Transparencia desde el primer día." 
              icon={<ShieldCheck className="w-6 h-6" />}
            />
            <FeatureItem 
              title="Garantía de Obra" 
              text="Instalaciones certificadas que cumplen con toda la normativa técnica y de seguridad vigente." 
        ...
        <Container>
          <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
            <div className="max-w-2xl">
              ...
              <H2 className="mt-4">Servicios de Fontanería para Reformas e Instalaciones</H2>
            </div>
